// GPGPU Simulation shader for wake particles streaming backward
...
void main() {
  ...
  // Respawn when life >= 1.0
  if (particle.a >= 1.0) {
    // Spawn at dolphin surface point with slight random offset
    ...
    if (distXY > 0.05) {
      ...
      // Spiral strength decreases with distance
      float spiralStr = uSpiralIntensity / (distXY * 0.5 + 0.3);
      // Add variation based on Z position (wake turbulence)
      spiralStr *= 1.0 + sin(toCenter.z * 1.5 + time * 2.0) * 0.4;
      spiral.xy = tangent * spiralStr;
    }
    
    // 5. Wake spreading - particles spread outward as they move back
    ...
    // 6. Oscillating motion (like particles caught in water waves)
    ...
    // Age particle - faster aging for particles further from dolphin
